...
function Commutes(configuration) {
  ...
  function initCommutesPanel() {
    const addCommutesButtonEls = commutesEl.destinationPanel?.querySelectorAll('.add-button') || [];
    addCommutesButtonEls.forEach(addButton => {
      addButton.addEventListener('click', () => {
        ...
        const radioButton = document.forms['destination-form']?.[travelModeId];
        if (radioButton) radioButton.checked = true;
      });
    });

    if (destinationPanelEl.scrollLeftButton) {
      destinationPanelEl.scrollLeftButton.addEventListener(
          'click', handleScrollButtonClick);
    }
    if (destinationPanelEl.scrollRightButton) {
      destinationPanelEl.scrollRightButton.addEventListener(
          'click', handleScrollButtonClick);
    }
    if (destinationPanelEl.list) {
  ...
  function initCommutesModal() {
    ...
    const destinationFormReset = function() {
      ...
      if (destinationModalEl.errorMessage) destinationModalEl.errorMessage.innerHTML = '';
    ...
    if (destinationModalEl.destinationInput) {
      ...
      if (destinationModalEl.addButton) {
      ...
      if (destinationModalEl.editButton) {
      ...
      if (destinationModalEl.cancelButton) {
      ...
      if (destinationModalEl.deleteButton) {
  ...
  function validateDestinationInput(destinationToAdd) {
    ...
    if (!isValidInput) {
      if (destinationModalEl.errorMessage) destinationModalEl.errorMessage.innerHTML = errorMessage;
  ...
  function buildDestinationCardTemplate(
    ...
    editButtonEl?.addEventListener('click', () => {
      if (destinationModalEl.title) destinationModalEl.title.innerHTML = 'Edit destination';
      if (destinationModalEl.destinationInput) destinationModalEl.destinationInput.value = destination.name;
...
function handleScrollButtonClick(e) {
  ...
  if (destinationPanelEl.container && cardWidth) {
...
function handlePanelScroll() {
  if (!destinationPanelEl.container) return;
...
// Initialize commute widget elements when DOM is ready
function initializeCommuteWidgetElements(widgetContainer) {
  commutesEl.map = widgetContainer.querySelector('.map-view');
  commutesEl.initialStatePanel = widgetContainer.querySelector('.commutes-initial-state');
  commutesEl.destinationPanel = widgetContainer.querySelector('.commutes-destinations');
...
window.Commutes = Commutes;
window.initializeCommuteWidgetElements = initializeCommuteWidgetElements;
