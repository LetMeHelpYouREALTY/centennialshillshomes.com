...
export interface CommuteWidgetProps {
...
export default component$<CommuteWidgetProps>(({ 
  center = { lat: 36.1898207, lng: -115.3130859 }, // Las Vegas default
  title = "Estimate Your Commute",
  description = "See travel time and directions from your potential new home",
  class: customClass = ""
}) => {
  ...
  useVisibleTask$(() => {
    ...
    // Create the commute widget HTML structure with SVG icons
    const widgetHTML = `
      <!-- SVG Icons -->
      <svg class="hide">
        <defs>
      ...
      <div class="commute-widget ${customClass}">
        <div class="commute-widget-header">
          <h3>${title}</h3>
          <p>${description}</p>
        </div>
        <div class="commute-widget-content">
          <div class="commutes">
            ...
            <div class="commutes-info">
              <div class="commutes-initial-state">
                ...
                <div class="description">
                  <h4 class="heading">Estimate commute time</h4>
    ...
    widgetRef.value.innerHTML = widgetHTML;

    // Initialize the commute widget when Google Maps is loaded
    const initCommuteWidget = () => {
      if (typeof window !== 'undefined' && (window as any).google && (window as any).google.maps && (window as any).initializeCommuteWidgetElements) {
        // Initialize widget elements
        (window as any).initializeCommuteWidgetElements(widgetRef.value);
    ...
    // Load Google Maps API and commutes widget script if not already loaded
    if (!document.querySelector('script[src*="maps.googleapis.com"]')) {
      const mapsScript = document.createElement('script');
      mapsScript.src = 'https://maps.googleapis.com/maps/api/js?key=AIzaSyD_gdHoCg0SdY29tFIk1vYY7dvgp9vgy_M&callback=initMap&libraries=places,geometry&solution_channel=GMP_QB_commutes_v3_c';
      mapsScript.async = true;
      mapsScript.defer = true;
      document.head.appendChild(mapsScript);

      // Set up the global initMap function
      (window as any).initMap = initCommuteWidget;
    }

    // Load commutes widget script if not already loaded
    if (!document.querySelector('script[src*="commutes-widget.js"]')) {
      const widgetScript = document.createElement('script');
      widgetScript.src = '/commutes-widget.js';
  ...
  return (
    <div ref={widgetRef} class="commute-widget-container">
      {/* SVG Icons will be injected here */}
    </div>
  );
});
